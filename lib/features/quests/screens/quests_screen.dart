...
class _SummaryBar extends ConsumerWidget { final List<Quest> quests; final dynamic user; const _SummaryBar({required this.quests, required this.user}); @override Widget build(BuildContext context, WidgetRef ref){ final total=quests.where((q)=>q.type==QuestType.daily).length; final done=quests.where((q)=>q.type==QuestType.daily && q.isCompleted).length; final focusMinutes=quests.where((q)=>q.category==QuestCategory.focus).fold<int>(0,(s,q)=>s+q.currentProgress); final practiceSolved=quests.where((q)=>q.category==QuestCategory.practice).fold<int>(0,(s,q)=>s+q.currentProgress); double planRatio=0; try{ if(user?.weeklyPlan!=null){ final today=DateTime.now(); final dateKey='${today.year.toString().padLeft(4,'0')}-${today.month.toString().padLeft(2,'0')}-${today.day.toString().padLeft(2,'0')}'; final completed=(user.completedDailyTasks[dateKey]??[]).length; final planTotalRaw=quests.where((q)=>q.id.startsWith('schedule_')).length; final planTotal=planTotalRaw==0?1:planTotalRaw; planRatio=completed/planTotal; }}catch(_){ }
...
class _QuestsScreenState extends ConsumerState<QuestsScreen>{
  ...
  Widget _buildQuestList(BuildContext context,List<Quest> quests,user){ final issue=ref.watch(questGenerationIssueProvider); final weekly=quests.where((q)=>q.type==QuestType.weekly).toList(); final completed=quests.where((q)=>q.isCompleted && q.type==QuestType.daily).toList(); final active=quests.where((q)=>!q.isCompleted && q.type==QuestType.daily).toList(); final completedIds=quests.where((q)=>q.isCompleted).map((q)=>q.id).toSet(); final allMap={for(final q in quests) q.id:q}; final analytics=ref.read(analyticsLoggerProvider); for(final q in quests){ if(!_loggedViews.contains(q.id)){ _loggedViews.add(q.id); if(user!=null){ analytics.logQuestEvent(userId:user.id, event:'quest_view', data:{'questId':q.id,'category':q.category.name,'difficulty':q.difficulty.name}); } }} final listChild=ListView(padding: const EdgeInsets.fromLTRB(16,16,16,120), children:[ _SummaryBar(quests:quests,user:user), if(issue) _IssueBanner(onClose: ()=> ref.read(questGenerationIssueProvider.notifier).state=false), _buildBannerIfNeeded(context,active,completed), if(weekly.isNotEmpty)...[ const _SectionHeader(title:'Haftalık Sefer'), ...weekly.map((q)=>QuestCard(quest:q,completedIds:completedIds,userId:user?.id,allQuestsMap:allMap,ref:ref)), const SizedBox(height:16)], if(active.isNotEmpty)...[ const _SectionHeader(title:'Günlük Emirler'), ...active.map((q)=>QuestCard(quest:q,completedIds:completedIds,userId:user?.id,allQuestsMap:allMap,ref:ref))], if(completed.isNotEmpty)...[ _SectionHeader(title:'Fethedilenler (${completed.length})'), ...completed.map((q)=>QuestCard(quest:q,completedIds:completedIds,userId:user?.id,allQuestsMap:allMap,ref:ref))], const SizedBox(height:24) ]).animate().fadeIn(duration:400.ms).slideY(begin:0.15); return RefreshIndicator(onRefresh: () async { if(user!=null){ await ref.read(questServiceProvider).refreshDailyQuestsForUser(user, force:true); ref.invalidate(dailyQuestsProvider);} }, child:listChild); }
