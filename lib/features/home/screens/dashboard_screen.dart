      ...
      body: SafeArea(
        child: userAsync.when(
          data: (user) {
            if (user == null) {
              return const Center(
                  child: Text('Kullanıcı verisi yüklenemedi.'));
            }
            ...
            final avgNet =
            testCount > 0 ? (user.totalNetSum / testCount) : 0.0;
            final bestNet =
            tests.isEmpty ? 0.0 : tests.map((t) => t.totalNet).reduce(max);
              ...
              children: [
                Padding(
                  ...
                  child: DashboardHeader(
                      greeting: _getGreeting(), name: user.name ?? ''),
                ),
                const SizedBox(height: 16),
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 16.0),
                  child: DashboardStatsRow(
                      avgNet: avgNet, bestNet: bestNet, streak: user.streak),
                ),
                const SizedBox(height: 24),

                // YENİ TASARIM: Yatay kaydırılabilir ana eylem merkezi
                const TodaysPlan(),
                ...
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 16.0),
                  child: MotivationalQuoteCard(quote: randomQuote),
                ),
                const SizedBox(height: 16),
              ]
